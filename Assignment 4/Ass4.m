...
text_cell = cell(round(dataset_length/100000)+2, n_epochs);
save_iter=1;
...
for i=1:n_epochs
    ...
    for e=e_values
        ...
        [RNN, adagrad, loss, hprev] = MiniBatchGD(X_chars_one_hot, Y_chars_one_hot, RNN, adagrad, hprev, eta);
        loss;
        if smooth_loss == 0
            smooth_loss = loss;
        end
        ...
        if (iter == 1) || (mod(iter,20000) == 0) %|| (e == dataset_length-seq_length)
            e;
            smooth_loss;
            iter
            %Y = synt_text(RNN, h0, X_chars_one_hot(:,1), 200);
            %DecodeString(Y, ind_to_char)
%             text_cell{save_iter, i} = DecodeString(Y, ind_to_char)
...
%     h0 = hprev;
%     save_iter=1;
%     for e=1:seq_length:dataset_length-seq_length
%         X_chars = book_data(e:e+seq_length-1);
%         Y_chars = book_data(e+1:e+seq_length);
% %         X_chars_one_hot = one_hot_encode_string(X_chars, char_to_ind);
% %         Y_chars_one_hot = one_hot_encode_string(Y_chars, char_to_ind);
...
%         smooth_loss = 0.999*smooth_loss + 0.001*loss
%         e
%         smooth_loss_vector(e +(i-1)*(dataset_length-seq_length)) = smooth_loss;
%         
% %         if (e == 1) || (mod(e,100000) == 0) || (e == dataset_length-seq_length)
% %             e
% %             smooth_loss
% %             Y = synt_text(RNN, h0, X_chars_one_hot(:,1), 200);
% %             text_cell{save_iter, i} = DecodeString(Y, ind_to_char)
...
function [RNN, adagrad, loss, hprev] = MiniBatchGD(X_chars_one_hot, Y_chars_one_hot, RNN, adagrad, hprev, eta)
...
for f = fieldnames(RNN)'
    %clipping
%     if ~isequal(max(min(grads.(f{1}), 5) -5), grads.(f{1}))
%         eps
%     end
    ...
    RNN.(f{1}) = RNN.(f{1}) - (eta*grads.(f{1}))./((adagrad.(f{1})+eps).^(0.5));
...
function xnext = sample_char(p)
cp = cumsum(p);
a = rand;
ixs = find(cp -a >0);
ii = ixs(1);
xnext = one_hot_encode_char(ii);
...
function one_hot_char = one_hot_encode_char(index)
