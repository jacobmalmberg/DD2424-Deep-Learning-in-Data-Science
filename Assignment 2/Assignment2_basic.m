...
% sanity check #3
trainX = trainX(1:20,1:20);
training_one_hot = training_one_hot(:,1:20);

validationX = validationX(:,1:20);
validation_one_hot = validation_one_hot(:,1:20);
...
%init matrices for cost per epoch
...
%3 test => J = 2.302...correct. BLIR SÄMRE med korrekt init 2.5362. med
%lambda = 0.01 erhålls 3.1448  med 100dp
...
% plot(1:n_epochs, train_cost, 1:n_epochs, validation_cost);
% vis(K, W);

function [X, Y, y] = LoadBatch(filename)
%pt 1
A = load(filename);
X = double(A.data');   %convert values to 0-1  
