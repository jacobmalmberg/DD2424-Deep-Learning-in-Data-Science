...
global p1;
%p = rand % set before training according to book
p1 = 0.7;
global p2;
p2 = 0.7;
...
lambda = 2e-4;%2.1622e-3; % best lambda
...
for i=1:n_epochs
    for j=1:N/n_batch
        ...
        %5.1 g jitter between 0.98 and 1.02
        jitter = 0.98 + rand(size(X_batch))*0.04 ;
        ...
        if (t ==1 ) || (ismember(t,x_axis)) %för grafen, bara räkna loss/cost/acc ibland  %(t == 1) || (mod(t,125) == 0)%(mod(t-2,130) == 0) || (t==1001) %(mod(t-2,190) == 0) || (t==4801)
            ...
            train_acc_t = ComputeAccuracy(trainX, trainY, W, b)
            train_acc(save_iter) = train_acc_t;
...
name_array = strrep(['Ex51e Cost m = ', num2str(m), ' lambda =', num2str(lambda)],'.',',');
make_plot(x_axis, train_cost, validation_cost, 'update step', 'cost', name_array)

name_array = strrep(['Ex51e Loss m = ', num2str(m), ' lambda =', num2str(lambda)],'.',',');
make_plot(x_axis, train_loss, validation_loss, 'update step', 'loss', name_array)

name_array = strrep(['Ex51e Accuracy m = ', num2str(m), ' lambda =', num2str(lambda)],'.',',');
make_plot(x_axis, train_acc, validation_acc, 'update step', 'loss', name_array)
...
function [P, s1, s2, h] = EvaluateClassifierWithDropOut(X, W, b)
% slide 74 in lect5
...
s1 = W{1}*X +b{1};
h = max(0,s1);
u1 = (rand(size(h)) < p1)/p1;
h = h.*u1;

s2 = W{2}*h + b{2};
u2 = (rand(size(s2)) < p2)/p2;
...
function acc = ComputeAccuracyWithDropout(X,y,W,b)
%5
[P, ~, ~, ~] = EvaluateClassifierWithDropOut(X,W,b);
[~,I] = max(P, [], 1); %argmax
N = size(y,1);
I=I';
acc = sum(y==I)/N;
